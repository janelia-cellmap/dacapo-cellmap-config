...
def create_yaml_with_crops(input_yaml_path, output_yaml_path,input_resolution,output_resolution):
    ...
    for dataset_name, dataset_info in data['datasets'].items():
        dataset_info["raw_path"] = get_right_resolutions(dataset_info["raw_path"], input_resolution)
        gt_pattern = dataset_info.get('gt_pattern')
        if gt_pattern and '[CROP]' in gt_pattern:
            # Detect actual crop names from the filesystem
            crops_found = detect_crops_from_gt_pattern(gt_pattern)
            if not crops_found:
                # If no matching crops found, you could leave it empty or skip
                dataset_info['crops'] = []
                continue
            
            # Build a new 'crops' list for this dataset
            dataset_info['crops'] = []
            for crop in crops_found:
                # Replace [CROP] in the pattern with the actual crop name
                crop_gt_path = gt_pattern.replace('[CROP]', crop)
                dataset_info['crops'].append({
                    'name': crop,
                    'gt_path': get_right_resolutions(crop_gt_path, output_resolution)
                })

    # Write out the updated YAML
